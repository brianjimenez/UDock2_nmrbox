...
public:
    ...
    void runThreadedEnergyCalc()
    { 
        if (_thread == nullptr)
        {
            _thread = new std::thread(&MolecularHandler::energyCalc, this);
            _runThread = true;
        }
    }

    void stopThreadedEnergyCalc()
    {
		if (_thread != nullptr)
		{
			_runThread = false;
...
//     }
    ...
    void dampMols(float elapsed, bool damp, Molecule * ignoreMol = NULL)
    {
        ...
        if(lastDamp == false && damp == true)
        {
            ...
            { 
                // TODO: print speeds.
                //std::cout << glm::to_string(angSpeed[i]) << " / " << glm::to_string(linSpeed[i]) << std::endl;
