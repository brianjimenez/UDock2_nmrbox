...
int docking_scene( SDL_Window* window, scene_docking_data& docking_data,NYTimer * g_timer){
    ...
    docking_data.elapsed = g_timer->getElapsedSeconds(true);  //( 1.0f / ImGui::GetIO().Framerate );
    //std::cout << g_timer->getElapsedSeconds(true) << " " << (1.0f / ImGui::GetIO().Framerate) << std::endl;
    ...
    if (docking_data.optim & (nbmol  >= 2 )){
        ...
        float updatePhy = glm::min(docking_data.elapsed,1.0f/ ImGui::GetIO().Framerate); //Bullet fait de la merde si sous 60hz...
