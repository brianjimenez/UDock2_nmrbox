...
void mousePollFunction(bool rotate,btDiscreteDynamicsWorld * dynamicsWorld, Camera  cam,float elapsed)
{
	...
	else
	{
        ...
        if (body)
        {
			...
			btVector3 angSpeed = body->getAngularVelocity();
			//btVector3 damping = (angSpeed/5.0f) * ImGui::GetIO().Framerate * elapsed;
...
void mousePollFunction(btRigidBody* body, const SDL_MouseMotionEvent& event, btDiscreteDynamicsWorld* dynamicsWorld, Camera  cam)
{		 
	if (body)
	{		
		if(body->getActivationState() != ACTIVE_TAG) body->setActivationState(ACTIVE_TAG);
		glm::vec3 impulseX = cam.UpVec;
		impulseX *= event.xrel;
		glm::vec3 impulseY = cam.RightVec;
		impulseY *= -event.yrel;

		glm::vec3 impulse = impulseX + impulseY;
		impulse *= 5.f;
