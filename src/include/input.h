...
void docking_input(scene_docking_data &docking_data)
{
    ...
    static int x_last_pos = 0;
    static int y_last_pos = 0;
    ...
    while (SDL_PollEvent(&windowEvent))
    {
        ...
        int xrel;// = windowEvent.motion.x - x_last_pos;
        int yrel;// = windowEvent.motion.y - y_last_pos;      

        //x_last_pos = windowEvent.motion.x;
        //y_last_pos = windowEvent.motion.y;
		
        //SDL_GetRelativeMouseState(&xrel, &yrel);
		xrel = windowEvent.motion.xrel;
		yrel = windowEvent.motion.yrel;      
...

        {
            ...
            case SDL_MOUSEMOTION:
            {
                if (windowEvent.motion.state == SDL_BUTTON_MMASK)
                {
                    ...
                    if (keybState[SDL_SCANCODE_LSHIFT]){
                         docking_data.d_cam->move(docking_data.d_cam->UpVec * (yrel *0.2f *(*docking_data.mouseSensitivity)));
                         docking_data.d_cam->move(docking_data.d_cam->RightVec * (xrel *0.2f *(*docking_data.mouseSensitivity)));
                    }
                    else{
                         docking_data.d_cam->move(docking_data.d_cam->Direction * (yrel *0.2f *(*docking_data.mouseSensitivity)));
                         docking_data.d_cam->move(docking_data.d_cam->RightVec * (xrel *0.2f *(*docking_data.mouseSensitivity))); 
                    }
                }
                if (windowEvent.motion.state == SDL_BUTTON_RMASK)
                {
                    ...
                    //     docking_data.d_cam->FPSRotation(-windowEvent.motion.xrel,-windowEvent.motion.yrel);
                        docking_data.d_cam->rotateAround(xrel * deltaTime * (*docking_data.mouseSensitivity));
                        if(*docking_data.invertedAxis){
                            docking_data.d_cam->rotateUpAround(-yrel * deltaTime *(*docking_data.mouseSensitivity));
                        }else{
                            docking_data.d_cam->rotateUpAround(yrel * deltaTime *(*docking_data.mouseSensitivity));
                        }
                    ...
                    //     docking_data.d_cam->TPSRotation(-windowEvent.motion.xrel,-windowEvent.motion.yrel);
                        //fps rotation
                        docking_data.d_cam->rotate(xrel * deltaTime *(*docking_data.mouseSensitivity));
                        // docking_data.deltaTime
                        if(*docking_data.invertedAxis){
                            docking_data.d_cam->rotateUp(-yrel* deltaTime *(*docking_data.mouseSensitivity));
                        }else{
                            docking_data.d_cam->rotateUp(yrel* deltaTime *(*docking_data.mouseSensitivity));
                        }
                ...
                {
                    mousePollFunction(selectedBody, windowEvent.motion, docking_data.mh->dynamicsWorld, *docking_data.d_cam);
                }
            ...
            case SDL_KEYUP:
            {
                ...
                // switch (windowEvent.key.keysym.sym)
                {
                    ...
                    {
                        docking_data.mh->stopThreadedEnergyCalc();
    ...
    if (FPS){
        ...
        //*docking_data.relativeMouseMode = SDL_FALSE;
    }
...

        {
            ...
            case SDL_MOUSEMOTION:
            {
                if (windowEvent.button.button == SDL_BUTTON_RMASK) {
                    ...
                    glm::vec2 rl;
                    if (*spaceship_data.invertedAxis) {
                        rl = glm::vec2(xrel * 5.f * deltaTime * (*spaceship_data.mouseSensitivity), -yrel * 5.f * deltaTime * (*spaceship_data.mouseSensitivity));
                    }
                    else {
                        rl = glm::vec2(xrel * 5.f * deltaTime * (*spaceship_data.mouseSensitivity), yrel * 5.f * deltaTime * (*spaceship_data.mouseSensitivity));
                    }
